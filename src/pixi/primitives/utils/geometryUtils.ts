...
import type { VamsObject, Vertex } from "@/types";

export function pxToWorld(px: number, worldScaleX?: number): number {
  return worldScaleX ? px / worldScaleX : 0.02;
}

export function bboxRadii(o: VamsObject): { cx: number; cy: number; rx: number; ry: number } {
...
export function hasUniformColor(o: VamsObject): boolean {
...
/**
 * Calculates the local bounding box of children relative to the group parent.
 * Since children now use LOCAL coordinates, we just need to calculate their bounding boxes
 * based on their local transforms and union them.
 */
export function getGroupLocalBounds(
  ...
  children.forEach(child => {
    // Child is already in LOCAL Group Space.
    // Calculate child's bounding box in its own local space, then transform by child's transform.
