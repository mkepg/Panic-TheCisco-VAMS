...
function tokenizeLine(line: string): Array<{ t: string; k: string }> {
  ...
  while (i < n) {
    ...
    // Preprocessor line (#include ...)
    if (i === 0 && /^\s*#/.test(line)) {
      out.push({ t: line, k: 'pp' });
      return out;
    }
    // Line comment
    if (ch === '/' && line[i + 1] === '/') {
      out.push({ t: line.slice(i), k: 'comment' });
      return out;
    }
    // String
    if (ch === '"') {
    ...
    // Number (including 1.0f, 2e3, hex)
    if (/[0-9]/.test(ch) || (ch === '.' && /[0-9]/.test(line[i + 1] || ''))) {
      const m = line.slice(i).match(/^[0-9]*\.?[0-9]+(e[+-]?[0-9]+)?[fFlLuU]*/);
      if (m) {
        out.push({ t: m[0], k: 'num' });
        i += m[0].length;
        continue;
      }
    }
    // Identifier / keyword / GL constant / function call
    if (/[A-Za-z_]/.test(ch)) {
...
const TokenizedLine = memo(function TokenizedLine({ line }: { line: string }) {
  ...
  return (
    <>
      {tokens.map((tok, i) => (
        <span key={i} className={`tk tk-${tok.k}`}>
          {tok.t}
        </span>
      ))}
    </>
...
const CodeViewer = memo(function CodeViewer({
  ...
  // Open search with Ctrl/Cmd+F inside the panel
  useEffect(() => {
    if (searchOpen) {
  ...
  const searchMatches = useMemo(() => {
    ...
    lines.forEach((line, i) => {
      if (line.toLowerCase().includes(q)) m.add(i);
    });
    return m;
  }, [lines, search]);
            ...
            <>
              {searchOpen ? (
                <div className="code-search">
                  ...
                  />
                  {search && (
                    <span className="search-count">{searchMatches.size}</span>
                  )}
                  <button
                    type="button"
      ...
      <div className="code-content">
        <div className="line-numbers" aria-hidden="true">
          {Array.from({ length: lineCount }, (_, i) => (
            <div
              key={i}
              className={`line-num ${highlightedLines.has(i) ? 'highlight' : ''} ${
        ...
        <pre>
          {lines.map((line, i) => (
            <div
              key={i}
              className={`code-line ${highlightedLines.has(i) ? 'highlight' : ''} ${
                searchMatches.has(i) ? 'match' : ''
              }`}
            >
