...
/**
 * @param objectsToDeclare  Objects that need draw_X() functions generated.
 * @param objectsToCallInDraw  Root-level objects whose draw calls go in draw().
 * @param visibleObjects  All visible objects — used for state globals (replaces the
 *                        old `allObjects` parameter so hidden objects are fully absent
 *                        from the generated output).
 */
export const generateAppOutput = (
  objectsToDeclare: VamsObject[],
  objectsToCallInDraw: VamsObject[],
  visibleObjects: VamsObject[],
  ...
  // State globals only for objects that will actually appear in the output
  fullCode += generateStateAndInput(objectsToDeclare.length > 0 ? visibleObjects : []);
