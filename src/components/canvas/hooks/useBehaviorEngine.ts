...
// ---------------------------------------------------------------------------
// Matrix math — number[][] 3×3 format used internally by the behavior engine
// ---------------------------------------------------------------------------
...
function getGlobalTransformMatrix(
  ...
  cache: Map<string, TransformState>
): number[][] {
...
function projectPolygon(axis: Point, vertices: Point[]): { min: number; max: number } {
  let min = Infinity;
  let max = -Infinity;
...
function checkPrimitiveCollision(
  obj1: VamsObject,
  obj2: VamsObject,
  allObjects: VamsObject[],
  cache: Map<string, TransformState>
...
export function useBehaviorEngine({
  ...
  const applyAction = useCallback(
    (obj: VamsObject, behavior: Behavior, deltaValue: number) => {
  ...
  const processCollisions = useCallback(() => {
    ...
    objects.forEach((obj) => {
      ...
      const collisionBehaviors = obj.behaviors.filter(
        (b) =>
          b.enabled &&
          (b.trigger === "COLLISION_START" || b.trigger === "COLLISION_STAY") &&
          b.triggerTargetId
      );
      ...
      collisionBehaviors.forEach((behavior) => {
        ...
        if (!collisionState) {
          collisionState = { isActive: false, lastTrigger: 0, interval };
          collisionStates.current.set(stateKey, collisionState);
        }

        const isImmediate =
          behavior.action === 'DESTROY' ||
          behavior.action === 'GAME_OVER' ||
          behavior.action === 'SET_COLOR';

        if (behavior.trigger === "COLLISION_START") {
          const isColliding = checkCollision(obj, targetObj, objects, transformCacheRef.current);

          // Bug 1 fix: the original code set collisionState.isActive = true on
          // first contact but never reset it when the objects separated. This made
          // COLLISION_START fire on every subsequent frame indefinitely — identical
          // to COLLISION_STAY. The semantic distinction ("trigger once on contact")
          // was completely lost.
          //
          // Fix: check the current collision state on every frame.
          // • If colliding and not yet active → mark active and fire once.
          // • If not colliding → reset isActive so the trigger can fire again
          //   the next time the objects make contact.
          if (isColliding) {
            if (!collisionState.isActive) {
              // First contact — fire once and mark as active.
              collisionState.isActive = true;
              collisionState.lastTrigger = now;
              applyAction(obj, behavior, behavior.value);
            }
            // While still colliding and already active: do nothing more.
            // (COLLISION_START must not keep firing — that is COLLISION_STAY's job.)
          } else {
            // Objects have separated — reset so the trigger can fire again on
            // the next contact event.
          ...
          if (isColliding) {
            if (!collisionState.isActive) {
              ...
              if (behavior.action === 'SET_COLOR') {
                collisionState.originalColors = obj.vertices.map((v) => ({
                  id: v.id,
                  color: v.color,
                }));
                applyAction(obj, behavior, behavior.value);
            ...
            if (collisionState.isActive) {
              ...
              if (behavior.action === 'SET_COLOR' && collisionState.originalColors) {
                ...
                if (firstColor) {
                  const isUniform = collisionState.originalColors.every(
  ...
  const handleObjectPointerDown = useCallback(
    (e: FederatedPointerEvent, objId: string) => {
